...
[`ghcr.io/docdb/postgres-documentdb:17-0.106.0-ferretdb-2.5.0`](https://ghcr.io/docdb/postgres-documentdb:17-0.106.0-ferretdb-2.5.0)
...
1. You can run the image with the following command:
   ...
   docker run -d \
     ...
     -p 5432:5432 \
     ghcr.io/docdb/postgres-documentdb:17-0.106.0-ferretdb-2.5.0
   ```
...
2. If you have `psql` installed, you can connect to the PostgreSQL with DocumentDB extension with the following command:
   ...
   ```sh
   docker run --rm -it ghcr.io/docdb/postgres-documentdb:17-0.106.0-ferretdb-2.5.0 \
     psql postgres://<username>:<password>@localhost:5432/postgres
   ```
