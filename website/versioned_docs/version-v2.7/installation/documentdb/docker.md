...
[`ghcr.io/docdb/postgres-documentdb:17-0.107.0-ferretdb-2.7.0`](https://ghcr.io/docdb/postgres-documentdb:17-0.107.0-ferretdb-2.7.0)
is recommended for most deployments.
It does not include FerretDB, so you must run it separately.
For a complete setup that includes FerretDB, see the [FerretDB installation](../docdb/docker.md).
...
1. You can run the image with the following command:
   ...
   docker run -d \
     ...
     -p 5432:5432 \
     ghcr.io/docdb/postgres-documentdb:17-0.107.0-ferretdb-2.7.0
   ```
...
2. If you have `psql` installed, you can connect to the PostgreSQL with DocumentDB extension with the following command:
   ...
   ```sh
   docker run --rm -it ghcr.io/docdb/postgres-documentdb:17-0.107.0-ferretdb-2.7.0 \
     psql postgres://<username>:<password>@localhost:5432/postgres
   ```

3. See [FerretDB Docker installation](../docdb/docker.md) for more details on connecting to FerretDB.

### Run development image

The development image for PostgreSQL with DocumentDB extension
[`ghcr.io/docdb/postgres-documentdb-dev`](https://ghcr.io/docdb/postgres-documentdb-dev)
...
Before [updating to a new FerretDB release](../docdb/docker.md#updating-to-a-new-version), it is critical to install the matching DocumentDB image first.
