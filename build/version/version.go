...
// Package version provides information about DocDB version and build configuration.
//
// # Extra files
//
// The following generated text files may be present in this (`build/version`) directory during building:
//   - version.txt (required) contains information about the DocDB version in a format
...
// # Go build tags
//
// The following Go build tags (also known as build constraints) affect builds of DocDB:
//
//	docdb_dev - enables development build (see below; implied by builds with race detector)
//
// # Development builds
//
// Development builds of DocDB behave differently in a few aspects:
...
// DocDB module path from go.mod.
const docdbModule = "github.com/hanzoai/docdb"
...
func initFromFiles() {
	...
	// 1. When we are working in the DocDB repo (building binaries, running integration tests, etc),
	//    those files are generated by `task gen-version`.
	// 2. When someone builds the DocDB binary as described in the package description.
...
// readBuildInfo returns DocDB version and commit from the build info.
// It also updates info.BuildEnvironment and info.Dirty when it is sure we are building DocDB,
// and not something that uses DocDB.
func readBuildInfo() (version, commit string) {
	...
	//
	// 1. Builds and unit tests in the DocDB repo (task build-host, task test-unit, etc).
	//      Version is not set even for tags due to https://github.com/golang/go/issues/72877.
	//    Path: "github.com/hanzoai/docdb/cmd/docdb"
	//      (or "github.com/hanzoai/docdb/build/version.test", etc)
	//    Main.Path: "github.com/hanzoai/docdb"
	//    Main.Version: "(devel)"
	//
	// 2. Builds with known module version (go install github.com/hanzoai/docdb/cmd/docdb@v2.0.0).
	//    Path: "github.com/hanzoai/docdb/cmd/docdb"
	//    Main.Path: "github.com/hanzoai/docdb"
	//    Main.Version: "v2.0.0"
	//
	...
	//    Main.Path: ""
	//    Main.Version: ""
	//    Deps.Path: "github.com/hanzoai/docdb"
	//    Deps.Version: "(devel)"
	//
	// 4. Integration tests (both in the test and in the command handler).
	//    Path: "github.com/hanzoai/docdb/integration.test"
	//    Main.Path: "github.com/hanzoai/docdb/integration"
	//    Main.Version: "(devel)"
	//    Deps: null
	//
	...
	//      Deps.Replace.Path: "../DocDB"
	//      Deps.Replace.Version: "(devel)"
	//
	// In short, DocDB version could be derived from the module version only when module version could be known,
	...
	// cases 1 and 2
	if buildInfo.Main.Path == docdbModule {
	...
	for _, dep := range buildInfo.Deps {
		...
		// vcs.revision and other settings refer to the repository that uses DocDB, not DocDB itself.
...
func init() {
	...
	if info.Version != unknown {
		if match := semVerTag.FindStringSubmatch(info.Version); match == nil || len(match) != semVerTag.NumSubexp()+1 {
			...
			msg += "See https://pkg.go.dev/github.com/hanzoai/docdb/build/version"
			panic(msg)
		}
	}
